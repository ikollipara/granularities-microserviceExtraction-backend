...
public class HistoryService {
    ...
    /**
     * Returns the history of a repository (@param repo) as a list of changes made
     * at each commit.
     * The return format is a list @link{ChangeEvent}, where each @{ChangeEvent}
    ...
    public List<ChangeEvent> computeChangeEvents(GitRepository repo) throws Exception {
        gitClient = new GitClient(repo, config);
    ...
    private List<ChangeEvent> handleDeletedFiles(List<ChangeEvent> events) {
        ...
        for (ChangeEvent event : events) {
            for (DiffEntry entry : event.getChangedfiles()) {
                if (entry.getNewPath().equals(config.DEV_NULL) && !entry.getOldPath().equals(entry.getNewPath())) { // file
                                                                                                                    // was
                                                                                                                    // deleted
