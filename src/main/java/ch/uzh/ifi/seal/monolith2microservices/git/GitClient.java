...
public class GitClient {
    ...
    private Repository rawRepository = null;
    ...
    public void cloneRepository() throws Exception {
        Git git = Git.cloneRepository().setURI(repo.getRemotePath())
                .setDirectory(new File(config.localRepositoryDirectory + "/" + repo.getName() + "_" + repo.getId()))
                .call();
        git.close();
    }
    ...
    public Repository getGitRepository() throws IOException {
        if (rawRepository == null) {
            ...
            rawRepository = new FileRepositoryBuilder().setGitDir(file).readEnvironment().findGitDir().build();
        }
    ...
    public List<ChangeEvent> getChangeEvents() throws Exception {
        ...
        git.close();
        walk.close();
        return changeHistory.stream().filter(changeEvent -> changeEvent.getChangedfiles().size() > 0)
